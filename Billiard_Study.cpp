...
////////////////////////// class MyBall //////////////////////////
// ボールの位置や速度を保持するクラス
// サブクラスを持たないので、デストラクタにはvirtualをつけない。
class MyBall {
private:
	float vx, vy;		// 速度
	float xx, yy;		// 位置
...
public:
	...
	bool update(DataSet *data, float tt);
	void draw(DataSet *data);
	float energy() { return vx*vx+vy*vy; }
};

MyBall::MyBall(DataSet *data, const TCHAR fname[], const TCHAR nn[]) {
	_tcscpy_s(name, _countof(name), nn);
...
bool MyBall::update(DataSet *data, float tt) {
...
void MyBall::draw(DataSet* data) {
	D3DXMATRIX mat;//3Dの変換行列(位置、回転、スケールを保存)
	
	// 初期化（単位行列にしてmatに保存）
	D3DXMatrixTranslation(&mat, xx, yy, 0.0f);//xx,yyの方向に移動(z軸は関係ないので0)
	// 作成した変換行列をデバイスに設定し、描画する位置を指定
...
void updateData(DataSet *data) {
	...
	const int divide = 10;
	float dt = 1.0f / divide;  // 各ステップの進行時間（tt）

	for(int ii = 0; ii < divide; ii++) {
		for(int bb = 0; data->ball[bb]; bb++) {
			data->ball[bb]->update(data, dt);
		}
	}
}
...
HRESULT drawData(DataSet *data) {
	...
	data->raxa->draw(data);
	//中身がカラになるまでループ
	for (int i = 0; data->ball[i]; i++) {
		data->ball[i]->draw(data);
	}
...
WindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	case WM_KEYDOWN:
		...
		if (wParam == ' ') mydata.flag[1] = !mydata.flag[1];//スペース	
...
void MainLoop(DataSet *data) {
	...
	data->ball[0] = new MyBall(data, _TEXT("data/raxa.x"), _TEXT("0"));
	data->ball[0]->move(0, 0);
	data->ball[0]->vel(0, 0);

	data->ball[1] = new MyBall(data, _TEXT("data/ball1.x"), _TEXT("1"));
	data->ball[1]->move(15.0f, 0.1f);
	data->ball[1]->vel(-0.8f, 0.3f);

	data->ball[2] = new MyBall(data, _TEXT("data/ball2.x"), _TEXT("2"));
	data->ball[2]->move(0.0f, 10);
	data->ball[2]->vel(1.0f, -0.1f);

	
	data->ball[3] = new MyBall(data, _TEXT("data/ball3.x"), _TEXT("3"));
	data->ball[3]->move(5.0f, 2);
	data->ball[3]->vel(1.0f, -0.1f);
	
	data->ball[4] = new MyBall(data, _TEXT("data/ball4.x"), _TEXT("4"));
	data->ball[4]->move(3.0f, 3);
	data->ball[4]->vel(1.0f, -1.2f);

	data->ball[5] = new MyBall(data, _TEXT("data/ball5.x"), _TEXT("5"));
	data->ball[5]->move(7.0f, 4);
	data->ball[5]->vel(0.1f, -0.1f);

	data->ball[6] = new MyBall(data, _TEXT("data/ball6.x"), _TEXT("6"));
	data->ball[6]->move(8.0f, 5);
	data->ball[6]->vel(0.2f, -0.3f);

	data->ball[7] = new MyBall(data, _TEXT("data/ball7.x"), _TEXT("7"));
	data->ball[7]->move(9.0f, 6);
	data->ball[7]->vel(0.5f, 0.1f);

	data->ball[8] = new MyBall(data, _TEXT("data/ball8.x"), _TEXT("8"));
	data->ball[8]->move(10.0f, 7);
	data->ball[8]->vel(0.3f, -0.1f);
